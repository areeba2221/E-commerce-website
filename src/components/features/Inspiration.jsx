...
import { inspirationSlides, inspirationContent } from "/src/data/Data";

import { ArrowRightIcon, ChevronRightIcon } from "/src/assets/Svg";

export default function Inspirations() {
  const [current, setCurrent] = useState(0);

  const next = () =>
    setCurrent((prev) => (prev + 1) % inspirationSlides.length);
  const prev = () =>
    setCurrent(
      (prev) =>
        (prev - 1 + inspirationSlides.length) % inspirationSlides.length,
    );

  const mainSlide = inspirationSlides[current];
  const nextSlide = inspirationSlides[(current + 1) % inspirationSlides.length];
  const nextNextSlide =
    inspirationSlides[(current + 2) % inspirationSlides.length];
    ...
    >
      <div className="flex items-center max-w-7xl mx-auto px-8 gap-0">
        <div className="w-[422px] pr-10 shrink-0">
          <h2 className="text-[40px] font-bold leading-tight text-[#3A3A3A] mb-4">
        ...
        <div
          className="flex-1 flex items-stretch gap-0 relative overflow-hidden"
          style={{ height: "580px" }}
        >
          <div

...
export default function ProductDetails() {
  ...
  if (!product)
    return <p className="text-center py-20">Product in not available</p>;
  ...
  return (
    <>
      <nav
        className="w-full flex items-center gap-3 text-[16px] text-[#9F9F9F] 
        bg-[#F9F1E7] px-6 py-6 rounded mb-10"
      >
        <Link to="/home" className="hover:text-black transition-colors ml-9">
          Home
        </Link>
        <RightArrow />
      ...
      <main className="max-w-360 mx-auto px-4 md:px-16 py-10 font-Poppins">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div className="flex flex-col-reverse md:flex-row gap-8">
            ...
            <div
              className="bg-[#F9F1E7] rounded-[10px] flex-1 max-w-105.75 h-135 
          ...
          <div className="flex flex-col">
            <h1 className="text-[42px] font-normal text-black mb-1">
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <StarIcon
                    key={s}
                    filled={s <= Math.round(product.ratings?.average || 0)}
                  />
                ))}
              </div>
            ...
            <div className="mb-6">
              <span className="text-[14px] text-[#9F9F9F] block mb-3">
                Size
              </span>
            ...
            <div
              className="flex flex-wrap items-center gap-4 pb-12 border-b 
              border-[#D9D9D9] mb-8"
            >
              <div
                className="flex items-center justify-between border border-[#B88E2F]
                rounded-[10px] w-30.75 h-16 px-4"
              >
                <button
                  onClick={() => setQuantity((prev) => Math.max(1, prev - 1))}
                  className="text-[16px] font-medium hover:text-[#B88E2F]"
                >
                  -
                </button>
